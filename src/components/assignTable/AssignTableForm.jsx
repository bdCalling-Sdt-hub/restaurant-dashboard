...
const AssignTableForm = () => {
  ...
  const onFinish = (values) => {
    if (Number(values.seats) > booking?.remainingBookedGuests) {
      ErrorToast(
        `You can book only ${booking?.remainingBookedGuests} seat(s) more`
      );
      return;
    }
  ...
  return (
    <>
      <div className=" bg-white w-[260px] p-4 rounded shadow-sm space-y-4 h-full">
        <div className="pt-2">
          <button onClick={handleGoBack} className="self-start flex items-center gap-2 text-sm font-medium text-blue-600 hover:text-blue-800 transition mb-3">
        ...
        {selectedTable && (
          <>
            <Form form={form} name="edit" layout="vertical" onFinish={onFinish}>
              <Form.Item
                name="seats"
                label={
